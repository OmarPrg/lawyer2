...
function Map({ children, styles, ...props }: MapProps) {
  ...
  useEffect(() => {
    ...
    const actualResolved = resolvedTheme === "dark" ? mapStyles.dark : mapStyles.light;

    const mapInstance = new MapLibreGL.Map({
      container: containerRef.current,
      style: actualResolved,
...
function DefaultMarkerIcon() {
  return (
    <div className="relative h-4 w-4 rounded-full border-2 border-white bg-[#4870bb] shadow-lg" />
  );
}
...
function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "relative rounded-xl border border-border bg-background p-4 text-foreground shadow-xl animate-in fade-in-0 zoom-in-95 font-primary",
        className
      )}
    >
      {closeButton && (
        <button
          type="button"
          onClick={handleClose}
          className="absolute top-2 right-2 z-10 rounded-sm opacity-70 transition-opacity hover:opacity-100"
        >
          <X className="h-4 w-4" />
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "flex items-center justify-center size-8 hover:bg-muted transition-colors",
...
function MapControls({
  ...
  return (
    <div
      className={cn(
        "absolute z-10 flex flex-col gap-1.5 text-foreground",
